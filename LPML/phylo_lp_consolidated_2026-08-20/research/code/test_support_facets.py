...
# Fixed direction family, identical dimensionality at every height.
DIRS=[]
# unit directions
for i in range(4):
...
for i,j in itertools.combinations(range(4),2):
    d=[0]*4;d[i]=d[j]=1;DIRS.append(tuple(d))
# pair differences, canonical i<j
for i,j in itertools.combinations(range(4),2):
    d=[0]*4;d[i]=1;d[j]=-1;DIRS.append(tuple(d))
# all-sign directions modulo global sign, first coordinate +1, excluding all+
...
def generic_desc(n,P,support=None):
    ...
    if support:
        ...
        DD=0
        for i,j in itertools.combinations(range(4),2):
            key=tuple(1 if k==i else -1 if k==j else 0 for k in range(4));lo,hi=support[key]
            DD=max(DD,abs(lo),abs(hi))
        D['D']=min(D['D'],DD)
    return D
...
if __name__=='__main__':
    rows=[]
    for N in (8,16):
        # focus on difficult heterogeneous patterns
        pats=g.patterns_for(N)
        for name in ('fourway_blocks','interleaved','skew'):
            pat=pats[name]
            ex=g.exact_best(pat)
            t=time.time();Ds=build_descriptors(pat);prep=time.time()-t
            t=time.time();v,nv,nc=build_global(pat,Ds);dt=time.time()-t
            rows.append({'N':N,'pattern':name,'exact':ex,'support_tensor':v,'ratio':v/ex,'vars':nv,'constraints':nc,'prep_seconds':prep,'solve_seconds':dt})
            print(rows[-1],flush=True)
    pd.DataFrame(rows).to_csv('/mnt/data/support_facet_results.csv',index=False)
