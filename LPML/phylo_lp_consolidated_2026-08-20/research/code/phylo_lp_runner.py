...
from phylo_substitution import BASES,BASE_INDEX,jc69,gtr_f,set_active_model
...
def run(args):
    ...
    for srank,z in enumerate(cand,1):
        ...
        opt=TL.optimize(args.branch_min,args.branch_max,args.branch_maxiter,args.branch_ftol)
    ...
    fields=['likelihood_rank','structural_rank','log_likelihood','bottleneck_support','secondary_score','direction',
            'optimization_success','optimization_iterations','likelihood_evaluations','newick','optimizer_message']
    ...
    summary={
      ...
      'preprocess_mode':args.preprocess_mode,'lp_relaxation_log_score':float(lp_score),'lp_solver_reported_s':float(solver_reported),
      'top_k_requested':args.top_k,'top_k_recovered':len(cand),'best_topk_log_likelihood':best['log_likelihood'],
      'best_tree_file':best_path,'topk_table':topk_path,'timings_file':timing_path,'timings':times,
    }
...
def make_parser():
    ...
    p.add_argument('--branch-min',type=float,default=1e-8,help='Minimum final integral-tree branch length')
    p.add_argument('--branch-max',type=float,default=10.0,help='Maximum final integral-tree branch length')
