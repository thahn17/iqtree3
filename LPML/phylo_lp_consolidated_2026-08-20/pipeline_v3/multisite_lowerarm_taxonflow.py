...
from phylo_substitution import get_active_model,is_jc_like,build_spectral_relaxation

spec=importlib.util.spec_from_file_location('vl',str(Path(__file__).resolve().parent/'multisite_variable_length_compressed.py'))
...
BRANCH_RELAXATION='auto'
SPECTRAL_SUPPORT_POINTS=7

def configure_branch_relaxation(mode='auto', support_points=None):
    global BRANCH_RELAXATION,SPECTRAL_SUPPORT_POINTS
    if mode not in ('auto','two-point','spectral'):
        raise ValueError('branch relaxation must be auto, two-point, or spectral')
    BRANCH_RELAXATION=mode
    if support_points is not None:
        n=int(support_points)
        if n<5:raise ValueError('spectral support_points must be at least 5')
        SPECTRAL_SUPPORT_POINTS=n

def branch_relaxation_mode():
    if BRANCH_RELAXATION=='auto':return 'two-point' if is_jc_like(get_active_model()) else 'spectral'
    return BRANCH_RELAXATION

def spectral_relaxation():
    return build_spectral_relaxation(get_active_model(),SPECTRAL_SUPPORT_POINTS)

def branch_transition_vertices(m,long_ratio=2.0):
    """List (feature_vector, transition_matrix) covering vertices of the active branch relaxation."""
    if branch_relaxation_mode()=='spectral':
        S=spectral_relaxation();return [(np.asarray(z,float),S.transition(z)) for z in S.vertices]
    Ps,Pl=trans_pair(m,long_ratio);return [(np.asarray([0.0]),Ps),(np.asarray([1.0]),Pl)]

def branch_feature_dim():return len(spectral_relaxation().mu) if branch_relaxation_mode()=='spectral' else 1

def configure_substitution_model(model):
    global PI,Q
    PI=np.asarray(model.pi,float).copy(); Q=np.asarray(model.q,float).copy(); vl.configure_substitution_model(model)
...
def raw_boxes(global_counts,long_ratio=2.0,variable_lengths=True,total_leaves=None):
    """Raw partial boxes by (count, A/C/G/T composition), with null leaves implicit.

    N/?/- and other non-ACGT characters are uninformative leaves with vector 1.
    Their count is total_leaves-sum(global_counts), so no fifth composition coordinate is needed.
    """
    g=tuple(map(int,global_counts));L=int(total_leaves if total_leaves is not None else sum(g));null_total=L-sum(g)
    if null_total<0: raise ValueError('Informative base counts exceed total leaves')
    raw={};by=defaultdict(list)
    if null_total>0:
        c=(0,0,0,0);v=np.ones(4);raw[(1,c)]=np.stack([v,v],axis=1);by[1].append(c)
    for b in range(4):
        if g[b]<=0:continue
    ...
    for m in range(2,L+1):
        ...
        for a in range(1,m//2+1):
            ...
            VA=branch_transition_vertices(a,long_ratio) if variable_lengths else [(np.asarray([0.0]),trans_pair(a,long_ratio)[0])]
            VB=branch_transition_vertices(b,long_ratio) if variable_lengths else [(np.asarray([0.0]),trans_pair(b,long_ratio)[0])]
            for c1 in by[a]:
                A=raw[(a,c1)]; alo,ahi=A[:,0],A[:,1]
                amlo=np.min(np.stack([P@alo for _,P in VA]),axis=0);amhi=np.max(np.stack([P@ahi for _,P in VA]),axis=0)
                for c2 in by[b]:
                    c=tuple(c1[k]+c2[k] for k in range(4))
                    if any(c[k]>g[k] for k in range(4)) or m-sum(c)>null_total:continue
                    B=raw[(b,c2)];blo,bhi=B[:,0],B[:,1]
                    bmlo=np.min(np.stack([P@blo for _,P in VB]),axis=0);bmhi=np.max(np.stack([P@bhi for _,P in VB]),axis=0)
...
def root_partition_values(counts,raw,a,long_ratio,variable_lengths,total_leaves=None):
    L=int(total_leaves if total_leaves is not None else sum(counts));b=L-a; vals={}
    VA=branch_transition_vertices(a,long_ratio) if variable_lengths else [(np.asarray([0.0]),trans_pair(a,long_ratio)[0])]
    VB=branch_transition_vertices(b,long_ratio) if variable_lengths else [(np.asarray([0.0]),trans_pair(b,long_ratio)[0])]
    for (mm,c1),A in raw.items():
        ...
        muA=np.max(np.stack([P@ahi for _,P in VA]),axis=0);muB=np.max(np.stack([P@bhi for _,P in VB]),axis=0)
...
class Model:
    ...
    def _build_taxon_flow(self):
        ...
        for t in range(L):
            ...
            lp.add_eq(row,1)
    def _prep(self):
        # Everything here depends only on the global site-count vector, not on which named
        # taxa carry those bases. Cache aggressively: real alignments often have many exact
        # site patterns but far fewer A/C/G/T count vectors.
        cache={}
        for pat in self.patterns:
            counts=tuple(pat.count(b) for b in BASES)
            if counts in cache:
                self.info.append(cache[counts]);continue
            raw=raw_boxes(counts,self.long_ratio,self.variable_lengths,total_leaves=self.L)
            scale={};ubraw={};ubmsg={}
            for m in range(1,self.L+1):
                ...
                if m<self.L:
                    VV=branch_transition_vertices(m,self.long_ratio) if self.variable_lengths else [(np.asarray([0.0]),trans_pair(m,self.long_ratio)[0])]
                    ubmsg[m]=np.max(np.stack([P@(hi/sc) for _,P in VV]),axis=0)
            rootvals={}
            bysplit={}
            for u in self.roots:
                a=int(self.units[u]['left_flow'])
                if a not in bysplit:bysplit[a]=root_partition_values(counts,raw,a,self.long_ratio,self.variable_lengths,total_leaves=self.L)
                rootvals[u]=bysplit[a]
            I={'counts':counts,'raw':raw,'scale':scale,'ubraw':ubraw,'ubmsg':ubmsg,'rootvals':rootvals}
            cache[counts]=I;self.info.append(I)
                ...
                for u,port in self.reqdef[1]:
                    ...
                    for j in range(4):
                        ...
                        for t,b in enumerate(pat):
                            if b not in BASE_INDEX or BASE_INDEX[b]==j:
                ...
                for j in range(4):
                    ...
                    if m==1:
                        if not self.taxon_flow:
                            rhs=sum(1.0 for b in pat if b not in BASE_INDEX or BASE_INDEX[b]==j);self.lp.add_eq(row,rhs)
                        else:self.lp.add_eq(row,sum(1.0 for b in pat if b not in BASE_INDEX or BASE_INDEX[b]==j)) # redundant, useful numerically
                    else:
