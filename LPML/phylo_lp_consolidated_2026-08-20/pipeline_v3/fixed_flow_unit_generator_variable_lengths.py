...
---------------------------------------
With --variable-length-paths, selected logical edges A--B are represented by
two alternative paths with the SAME fixed flow.  By default the scope is
"branches": terminal taxon edges and non-root split upper arms only.  Use
--variable-length-scope all to retain the original behavior on every logical edge.
...
class LibraryBuilder:
    def __init__(
        ...
        *,
        ...
        variable_length_scope: str = "branches",
    ):
        if not leaves:
        ...
        if variable_length_scope not in ("branches", "all"):
            raise ValueError("variable_length_scope must be 'branches' or 'all'")

        self.leaves = leaves
        self.L = len(leaves)
        self.variable_length_paths = variable_length_paths
        self.long_path_edges = long_path_edges
        self.variable_length_scope = variable_length_scope
    ...
    def _edge_has_variable_length(self, le: LogicalEdge) -> bool:
        if not self.variable_length_paths:
            return False
        if self.variable_length_scope == "all":
            return True
        # One likelihood-bearing branch per subtree unit.  Count-L upper arms
        # terminate at the root and therefore have no branch above them.
        return (
            le.logical_edge_id == "taxon_edge"
            or (le.logical_edge_id == "upper_arm" and le.edge_flow < self.L)
        )
    ...
    def _materialize_internal_paths(self, le: LogicalEdge):
        ...
        has_variable_length = self._edge_has_variable_length(le)
        add_path("short" if has_variable_length else "base", [le.endpoint_a, le.endpoint_b])
    ...
    def verify(self):
        ...
        for le in self.logical_edges:
            ...
            expected = 2 if self._edge_has_variable_length(le) else 1
            if len(ps) != expected:
    ...
    # ----- branch-length choice metadata -----------------------------------

    def length_choice_groups(self) -> Dict[str, dict]:
        """Return independent short/long path groups for every logical edge.

        The physical library materializes both path alternatives whenever
        variable_length_paths is enabled.  ``likelihood_bearing`` marks the
        one edge per subtree unit used by the likelihood LP as the biological
        branch-length choice: taxon_edge for leaves and upper_arm for split
        units.  Router/left/right logical edges remain available geometrically
        but are not assigned independent evolutionary transitions by default.
        """
        if not self.variable_length_paths:
            return {}
        by_group: Dict[str, List[InternalPath]] = defaultdict(list)
        for path in self.internal_paths:
            by_group[path.hybrid_group].append(path)
        logical_by_group = {
            f"{le.unit_id}:{le.logical_edge_id}": le for le in self.logical_edges
        }
        out: Dict[str, dict] = {}
        for group, paths in sorted(by_group.items()):
            le = logical_by_group[group]
            by_option = {x.path_option: x for x in paths}
            if "short" not in by_option and "long" not in by_option:
                continue
            if "short" not in by_option or "long" not in by_option:
                raise AssertionError(f"Incomplete variable-length group {group}")
            short = by_option["short"]
            long = by_option["long"]
            likelihood_bearing = (
                le.logical_edge_id == "taxon_edge"
                or (le.logical_edge_id == "upper_arm" and le.unit_id not in self.root_candidates)
            )
            if le.logical_edge_id == "taxon_edge":
                role = "terminal_branch"
            elif le.logical_edge_id == "upper_arm":
                role = "subtree_upper_branch"
            elif le.logical_edge_id in ("left_arm", "right_arm"):
                role = "unit_internal_arm"
            elif le.logical_edge_id == "through":
                role = "router_through"
            else:
                role = "other"
            out[group] = {
                ...
                "edge_flow": le.edge_flow,
                "role": role,
                "likelihood_bearing": likelihood_bearing,
                "short_path_edges": short.path_length_edges,
                "long_path_edges": long.path_length_edges,
                "length_ratio": long.path_length_edges / short.path_length_edges,
                "short_nodes": short.path_nodes,
                "long_nodes": long.path_nodes,
            }
        return out
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "variable_length_scope": self.variable_length_scope if self.variable_length_paths else None,
            ...
            "length_choice_groups": len(self.length_choice_groups()),
            "likelihood_length_groups": sum(
                1 for g in self.length_choice_groups().values() if g["likelihood_bearing"]
            ),
    ...
    def write_library(self, out_dir: Path) -> List[Path]:
        ...
        payload = {
            ...
            "length_choice_groups": self.length_choice_groups(),
        }
        ...
        lines = [
            ...
            f"variable-length scope = {s['variable_length_scope']}",
    ...
    def write_routing(
        ...
        *,
        ...
        active_length_choices: Optional[Dict[str, str]] = None,
    ) -> List[Path]:
        ...
        if self.variable_length_paths:
            groups = self.length_choice_groups()
            supplied = active_length_choices or {}
            unknown = sorted(set(supplied) - set(groups))
            if unknown:
                raise ValueError(f"Unknown length-choice groups: {unknown[:5]}")
            for group, meta in groups.items():
                if meta["unit_id"] not in active_ids:
                    continue
                choice = supplied.get(group, active_length_choice)
                if choice not in ("short", "long"):
                    raise ValueError(f"Length choice for {group} must be short or long")
                choice_map[group] = choice
        else:
            for p in self.internal_paths:
                if p.unit_id in active_ids:
                    choice_map[p.hybrid_group] = "base"
        active_ipaths = [
            asdict(p) for p in self.internal_paths
            if p.unit_id in active_ids and choice_map.get(p.hybrid_group) == p.path_option
        ]
        active_iconns = [
            asdict(c) for c in self.internal_connections
            if c.unit_id in active_ids and choice_map.get(c.hybrid_group) == c.path_option
        ]
        chosen = active_length_choice if self.variable_length_paths else "base"
        ...
        payload = {
            ...
            "active_length_choices": choice_map,
            **routing,
        ...
        lines = [
            ...
            f"default active internal path choice = {chosen}",
            f"independent active length groups = {len(choice_map)}",
            "",
...
def parse_args():
    ...
    p.add_argument(
        "--variable-length-scope", choices=["branches", "all"], default="branches",
        help=("Which logical edges receive short/long alternatives: biological "
              "split-unit left/right lower biological branches only (default), or all logical edges."),
    )
    p.add_argument(
    ...
    p.add_argument(
        "--active-length-map", default=None,
        help=("Optional JSON file mapping hybrid_group (UNIT:logical_edge) to "
              "short/long. Unspecified groups use --active-length-choice."),
    )
    return p.parse_args()
...
def main():
    ...
    builder = LibraryBuilder(
        ...
        variable_length_scope=args.variable_length_scope,
    )
    builder.build()
    ...
    active_length_choices: Optional[Dict[str, str]] = None
    if args.active_length_map:
        active_length_choices = json.loads(Path(args.active_length_map).read_text(encoding="utf-8"))
        if not isinstance(active_length_choices, dict):
            raise SystemExit("--active-length-map must contain a JSON object")
    ...
    if parsed_root is not None and tree_text is not None:
        ...
        routing_paths = builder.write_routing(
            ...
            active_length_choices=active_length_choices,
        )
    ...
    if s['variable_length_paths']:
        print(f"  variable length scope:      {s['variable_length_scope']}")
        print(f"  independent length groups: {s['length_choice_groups']}")
        print(f"  likelihood length groups:  {s['likelihood_length_groups']}")
