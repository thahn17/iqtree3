...
//    save_all_br_lens = false;
    ...
    // [search-experiments hook] zero the experiment counters and clock; see
    // search_experiments/iqtree_search_experiments.cpp
    initSearchExperimentState();
...
void IQTree::initSettings(Params &params) {
    ...
    // [search-experiments hook] --spr-refine/--nni-refine and the other experiment
    // flags: parsed and validated here rather than at
    // the first perturbation, so a bad flag string is reported before the
    // run spends anything on model fitting or starting trees. It must also
    // run BEFORE setLikelihoodKernel below, which is where safe_numeric is
    // latched from params.lk_safe_scaling -- "reopt" needs the safe kernel
    // and initRefinement is what turns it on.
    initRefinement(params);
...
string IQTree::doRandomNNIs(bool storeTabu) {
    ...
    // [search-experiments hook] --perturb-slack: bound how far one kick move may drive
    // the tree downhill. Inert (slack.active == false) without the flag.
    sprsearch::KickSlack slack;
    beginNNIKickSlack(slack);
    ...
    while (cntNNI < numRandomNNI) {
        ...
        if (constraintTree.isCompatible(randNNI)) {
            ...
            doNNI(randNNI);
            // [search-experiments hook] --perturb-slack may undo this move and redraw
            // the same slot, or give the slot up
            if (slack.active) {
                sprsearch::KickSlack::Verdict verdict = judgeNNIKickMove(slack, randNNI);
                if (verdict == sprsearch::KickSlack::REDRAW)
                    continue;
                if (verdict == sprsearch::KickSlack::GIVE_UP) {
                    cntNNI++;
                    continue;
                }
            }
    ...
    endNNIKickSlack(slack);   // [search-experiments hook]
    if (verbose_mode >= VB_MAX)
...
//    }
    ...
    // [search-experiments hook] resets the annealing clock, and under --spr-continuous
    // runs the single long SPR hill-climb that replaces the perturb/refine
    // loop below (hence the two !params->spr_continuous guards)
    beginSearchExperiments();

    bool early_stop = stop_rule.meetStopCondition(stop_rule.getCurIt(), cur_correlation);
    if (!early_stop && !params->spr_continuous) {
        cout << "--------------------------------------------------------------------" << endl;
    ...
    while (!params->spr_continuous && !stop_rule.meetStopCondition(stop_rule.getCurIt(), cur_correlation)) {
         ...
         * Perturb the tree
         *---------------------------------------*/
        // [search-experiments hook] advance the shared annealing clock read by
        // "slack anneal" and "--accept-dist ... anneal"
        updateSearchProgress();
        doTreePerturbation();

        /*----------------------------------------
         * Optimize tree with NNI -- or, under --spr-refine, with the SPR
         * hill-climber ([search-experiments hook] IQTree::doSPRSearch, in
         * search_experiments/iqtree_search_experiments.cpp). Both leave
         * the result in curScore and in this tree object.
         *----------------------------------------*/
        pair<int, int> nniInfos(0, 0); // <num_NNIs, num_steps>
        if (params->refine_mode == REFINE_SPR)
            doSPRSearch();
        else
            nniInfos = doNNISearch();
        // [search-experiments hook] "record"/"trajectory"/"findopt" bookkeeping; only
        // counts the iteration unless one of those was asked for
...
//            MPIHelper::getInstance().sendMsg(LOGL_CUTOFF_TAG, convertDoubleToString(logl_cutoff));
    ...
    // [search-experiments hook] end-of-run "sweep" pass and final "record" row;
    // both inert unless --spr-refine/--nni-refine asked for them
    finishSearchExperiments();
    ...
    // [search-experiments hook] writes <prefix>.searchstats.txt and, when those
    // flags were given, the accept-dist / slack summaries
    reportSearchExperiments();
...
double IQTree::doTreePerturbation() {
    if (iqp_assess_quartet == IQP_BOOTSTRAP) {
        ...
        if (params->snni) {
            ...
            if (doExperimentPerturbation()) {
                // [search-experiments hook] --accept-dist suppressed the kick, or
                // --spr-perturb applied an SPR kick in place of this one
            } else if (Params::getInstance().iqp) {
                doIQP();
...
pair<int, int> IQTree::optimizeNNI(bool speedNNI) {
    ...
    // [search-experiments hook] per-call --accept-dist state: the stall counter and
    // the best tree this call passes through
    sprsearch::NNIAcceptState acceptWalk;
...
//        optimizeBranches();
    ...
    if (accept_dist.enabled)
        beginNNIAcceptWalk(acceptWalk);   // [search-experiments hook]
...
//        cout << "numSteps = " << numSteps << endl;
        ...
        // [search-experiments hook] --accept-dist lets the walk continue (a bounded
        // number of steps) past a step that lost ground
        if (accept_dist.enabled) {
            if (nniAcceptWalkShouldStop(acceptWalk, curScore, oldScore))
                break;
    ...
    // [search-experiments hook] --accept-dist: hand back the best tree this call
    // passed through, not wherever its walk happened to stop
    if (accept_dist.enabled)
        endNNIAcceptWalk(acceptWalk);
    bool showMsg = false;
    if (verbose_mode >= VB_MED) {
        showMsg = true;
    }
...
void IQTree::evaluateNNIs(Branches &nniBranches, vector<NNIMove>  &positiveNNIs) {
    for (Branches::iterator it = nniBranches.begin(); it != nniBranches.end(); it++) {
        ...
        // [search-experiments hook] counts the evaluation for "record" and, under
        // --accept-dist, may admit an NNI that loses ground
        if (acceptNNICandidate(nni.newloglh, curScore)) {
