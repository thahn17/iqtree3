...
void IQTree::initSettings(Params &params) {
    ...
    // --spr-refine/--nni-refine: parsed and validated here rather than at
...
//    }
    ...
    // --spr-continuous: this replaces the whole perturb/refine loop below
    // with a single uninterrupted SPR hill-climb from the best candidate
    // tree. See Params::spr_continuous.
    ...
    while (!params->spr_continuous && !stop_rule.meetStopCondition(stop_rule.getCurIt(), cur_correlation)) {
        ...
        /*----------------------------------------
         * Refine the perturbed tree back to a local optimum -- NNI by
         * default, or the shared SPR hill-climber under --spr-refine
         * (see IQTree::doSPRSearch and tree/sprsearch.h). Both leave the
         * result in curScore and in this tree object, so everything
        ...
        // "record"/"trajectory"/"findopt" bookkeeping for whichever
        // refiner just ran -- a no-op unless one of those was asked for
...
void IQTree::evaluateNNIs(Branches &nniBranches, vector<NNIMove>  &positiveNNIs) {
    for (Branches::iterator it = nniBranches.begin(); it != nniBranches.end(); it++) {
        ...
        // branch, so one iteration here is two candidate evaluations --
        // the same thing scoreTrialSPRMove counts one of per trial regraft.
